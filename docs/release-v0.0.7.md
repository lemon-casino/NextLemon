...
- 新增 Playwright e2e 冒烟测试：`npm run test:e2e` 三条用例覆盖应用加载/双模式切换、创作画布添加文本素材与撤销、画布助手面板打开；首次运行需 `npx playwright install chromium`。
- 修复模式切换浮窗遮挡创作画布右侧面板切换按钮的缺陷（创作模式下浮窗左移避让侧栏头部）。
- 新增生成结果自动沉淀（含执行占位）：创作画布工具栏"自动沉淀"开关（默认关闭），开启后工作流节点执行即在画布放置"生成中"占位实例，完成后产物自动保存为素材并替换占位（并排落位），失败自动清理。
- 文本素材编辑支持 @ 提及选择：输入 @ 弹出素材列表（标签/标题过滤、键盘与点击插入 `@[asset_N]`）。
...
- UI 自动化：已有单元测试、构建验证和 Playwright e2e 冒烟（`npm run test:e2e`，覆盖模式切换、创作画布添加文本/撤销、助手面板打开；e2e 过程中发现并修复了模式切换浮窗遮挡创作面板切换按钮的缺陷）。画布拖拽、Agent 对话流等深层交互的 e2e 仍可继续扩展。
- 同步真实环境：WebDAV 基础服务已实现，仍需要真实 WebDAV 服务、多端同步和冲突处理验收。
- 参考项目设计未吸收项：@提及输入推广到工作流 Prompt 节点编辑器、等间距吸附、加载动画占位，已在方案文档 6.2 节列为后续增强路线。
